...
const config: NextConfig = {
  ...
  // lets Next compile them in place so the landing renders the exact files
  // the registry will ship, same reasoning as apps/playground/next.config.ts.
  experimental: {
    externalDir: true,
  },
   ...
   *
   * `pnpm build` at the repo root runs `next build` for this app as part of
   * the turbo pipeline. With a shared `.next`, doing that while `next dev`
   * is running replaces the dev server's chunks underneath it, and the next
