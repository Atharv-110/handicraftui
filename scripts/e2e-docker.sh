#!/usr/bin/env bash
#
# Runs the Playwright `e2e` project inside the exact pinned image CI uses
# (cycle 003 §2.3), so a screenshot generated or compared here matches CI's
# without a human owning a platform-sniffing step. Cycle 003 §15.3's fix
# brief, replacing a `package.json` one-liner that corrupted the host
# `node_modules`.
#
...
mapfile -t PKG_RELDIRS < <(pnpm -r list --depth -1 --json | node -e '
...
#
# `bash -lc SCRIPT bash "$@"` is the standard idiom for forwarding a caller's
# arguments through a `-c` string: everything after SCRIPT becomes that
# bash's own positional parameters ($0 is the literal "bash" placeholder,
# $1... are the real arguments), so `"$@"` inside SCRIPT expands to them.
# Without this, `pnpm test:e2e:docker -g P3`'s `-g P3` would land after the
# whole docker invocation instead of reaching `pnpm test:e2e`, and the filter
# would be silently dropped — exactly the failure QA reproduced.
docker run --rm --ipc=host \
  -v "$ROOT":/w \
  "${VOLUME_ARGS[@]}" \
  -w /w \
  "$IMAGE" \
  bash -lc 'corepack enable && pnpm install --frozen-lockfile --store-dir /pnpm-store && HC_SNAPSHOT_ENV=docker pnpm test:e2e "$@"' bash "$@"
