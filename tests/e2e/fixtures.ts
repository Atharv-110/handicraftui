...
export interface HcFixture {
   ...
   * At `"lite"` it is the reverse: at least one `.hc-frame`, every one
   * resolved to `data-hc-fidelity="lite"`, and zero mounted `.hc-sketch-svg`
   * — proving tier 1 painted and tier 2 never took over. Auto-retry does the
   * waiting in both cases; nothing in this file calls `waitForTimeout`.
   */
   ...
   * own method rather than folded into `goto()`, since the font wait is a
   * matrix-screenshot concern the other 64 specs never needed.
   */
    ...
    const fixture: HcFixture = {
      async goto(state = {}, path = "/") {
        ...
        if ((state.fidelity ?? "high") === "lite") {
          ...
          // Cycle 007, same mechanism as `gotoSpecimen` below. The negative
          // assertion that follows is true before tier 2 could have mounted;
          // this one is only true once the provider has published that it will
          // not mount at all.
      ...
      async gotoSpecimen(cell) {
        ...
        if (cell.tier === "lite") {
          // Cycle 007. This used to be a two-frame settle anchored on page
          // load, and every version of that guard measured a clock. Three
          // instruments in cycle 004 found the tier-2 mount racing `load`
          // rather than following it, so no frame offset was ever a boundary.
          // The frame now publishes the tier it resolved to, and `lite` is
          // published only once tier 2 cannot run, so the wait below reads a
          // decision — one that is already in the server HTML before hydration.
          //
          // F14, cycle 004 iteration 3, still stands: this branch only ever
          // runs for block B, which is Button-only, so a `.hc-frame` always
          // exists to find. That is an assumption about the branch rather than
          // a fact about the type, and the day a tier-1 Label cell is added it
          // fails loudly naming the locator, which is a gate working.
          await expect(page.locator(".hc-frame")).not.toHaveCount(0);
          await expect(page.locator('.hc-frame:not([data-hc-fidelity="lite"])')).toHaveCount(0);
          await expect(page.locator(".hc-sketch-svg")).toHaveCount(0);
        } else {
